...
public record RaceRound(int round) {
    ...
    public RaceRound {
        if (round < MIN_ROUND) throw new RaceException(RaceRoundErrorCode.NEGATIVE_ROUND);
        if (round > MAX_ROUND) throw new RaceException(RaceRoundErrorCode.MAX_ROUND_EXCEEDED);
    }
}
